...
Class assistant_Class{
    ...
    //Fonction permettant la création d'un Demandeur 
    function createDemanndeur($matri_et, $nom_et, $prenom_et, $mail_et,$tel_et,$univ_et, $spec_et ,$idAs,$type_De,$etat_dem){
        
        $sql = "INSERT INTO Demandeur (matricule_etu, nom_etu, prenom_etu, email_etu, numtel_etu, univ_etu, specialite_etu, id_ast, statut_ast, etat_dem) VALUES ('$matri_et', '$nom_et','$prenom_et', '$mail_et', '$tel_et', '$univ_et', '$spec_et', '$idAs', '$type_De','$etat_dem') ";
    ...
    function getDemandeurs(){             
        $sql ="SELECT * FROM Demandeur ORDER BY id_ast DESC";
        return pg_query($sql);
    } 

    //Fonction permettant de lister tous les assistants par matricule
    function getDemandeurById($matricule_etu){    
  
        $sql ="SELECT * FROM Demandeur WHERE id_ast='".$this->cleanData($matricule_etu)."'"; 
        return pg_query($sql);
        
    } 
    //Fonction permettant de recuperer le dernier matricule 
    function getDemandeurLastId(){    
  
      
        //$sql ="SELECT * FROMassistant LIMIT 1 ORDER BY id_Assistant DESC"; 
        $sql = "SELECT id_ast FROM Demandeur ORDER BY id_ast ASC";
        $Assistants = pg_query($sql); 
        while($Demandeur = pg_fetch_object($Assistants)):
            $lastid = $Demandeur->id_ast;
        endwhile;
        return $lastid;
        
    } 
    //Fonction permettant de transformer un matricule en un varchar
    function getCleanedId_demandeur($id)
    {
    ...
    function deletedemandeur($matricule_etu){    
  
         $sql ="DELETE FROM Demandeur  WHERE id_ast='$matricule_etu'"; 
        if(pg_query($sql))
        {
            $res = true;
        }
    ...
    function updateDemandeur($matricule_etu,$nom_et,$prenom_et,$email_et,$numtel_et,$univ_et,$specialite_et){       
        
        $sql = "UPDATE Demandeur SET nom_etu= '$nom_et, prenom_etu = '$prenom_et',email_etu = '$email_et',numtel_etu = '$numtel_et', univ_etu= '$univ_et',specialite_etu= '$specialite_et' WHERE matricule_etu= '$matricule_etu' " ;
        $res = false;
...
while($Demandeur = pg_fetch_object($Assistants)):
    echo $Demandeur->longitude . '<br>';
    echo $Demandeur->latitude . '<br>';
    echo $Demandeur->etatb . '<br>';
    endwhile;  
*/
?>